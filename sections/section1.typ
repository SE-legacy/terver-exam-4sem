...
Борелевская $sigma$-алгебра $cal(B)$ --- это множество всех возможных подмножеств на числовой прямой, которые представимы ввиде конечного или счетного объединения интервалов/промежутков $cal(B)(RR) = {[a, b], space (a, b), space [a, b), space (a, b], space(- infinity, a), space (- infinity a], space (b, + infinity), space [b, +infinity)}$

== Теорема Каратеодори (о продолжении вероятностной меры)

Пусть $Q(A)$ --- счётно-аддитивная вероятностная мера на алгебре $cal(A)$. Тогда существует единственная счётно-аддитивная веротностная мера $P(A)$, заданная на минимальной $sigma$-алгебре над классом $cal(A)$, являющаяся продолжением меры $Q(A)$, т.е. $P(A) = Q(A) space forall A in cal(A)$
...
События $A$ и $B$ независимы $ <=> P(A | B) = P(A)$
